...
// ─── QR Decoder via jsQR (loaded from CDN) ───────────────────────────────────
...
// ─── QR Canvas (uses qrcode npm package) ─────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const drawQR = (ctx) => {
      try {
        ...
        // Data dots (skip finder pattern cells)
        ctx.fillStyle = fg
    ...
    if (logo === 'brand') drawLogo(ctx, BRAND_LOGO_SRC)
    else if (logo === 'custom' && logoUri) drawLogo(ctx, logoUri)
...
function Icon({ name, size = 16, color = 'currentColor', style }) {
  const paths = {
    ...
    'grid-outline':           <><rect x="3" y="3" width="7" height="7" rx="1" stroke={color} strokeWidth="1.6" fill="none"/><rect x="14" y="3" width="7" height="7" rx="1" stroke={color} strokeWidth="1.6" fill="none"/><rect x="3" y="14" width="7" height="7" rx="1" stroke={color} strokeWidth="1.6" fill="none"/><rect x="14" y="14" width="7" height="7" rx="1" stroke={color} strokeWidth="1.6" fill="none"/></>,
    'grid':                   <><rect x="3" y="3" width="7" height="7" rx="1" fill={color}/><rect x="14" y="3" width="7" height="7" rx="1" fill={color}/><rect x="3" y="14" width="7" height="7" rx="1" fill={color}/><rect x="14" y="14" width="7" height="7" rx="1" fill={color}/></>,
...
// ─── Logo (actual PNG from mobile, SVG fallback) ──────────────────────────────
function PaisaWordmark() {
  const [imgFailed, setImgFailed] = useState(false)
  return (
    <div className="wordmark">
      {!imgFailed ? (
        <img src="/icon.png" width="28" height="28" alt="" className="wordmark-img"
    ...
    <div className="field">
      ...
      <div className="field-wrap" style={focused && accentColor ? { borderColor: accentColor + '55' } : {}}>
        <span className="field-icon">
          <Icon name={iconName} size={15} color={focused && accentColor ? accentColor : 'var(--text3)'} />
        </span>
...
function UploadDropZone({ onQRDetected, accentColor }) {
  ...
  const [status, setStatus] = useState(null) // null | 'scanning' | 'success' | 'error'
    ...
    <div className="qr-card">
      <div className="qr-card-header">
        ...
        {hasData && name
          ...
          : <span className="qr-placeholder">Fill in details →</span>}
      </div>
      ...
      <div className="qr-stamp">
        <div className="stamp-dot" /><span>Paisa Studio</span>
      </div>

      <div className="qr-actions">
        <button className="btn-save" disabled={!hasData} onClick={onDownload}>
          <Icon name="download-outline" size={14} color={hasData ? '#0f0f10' : 'var(--text3)'} />
          Save QR
        </button>
        <button className="btn-share" disabled={!hasData} onClick={onDownload}>
          <Icon name="share-outline" size={14} color="var(--text2)" />
          Share
        </button>
      </div>
    </div>
...
function StylePanel({ fg, bg, dotStyle, cornerStyle, logo, logoUri, onFg, onBg, onDotStyle, onCornerStyle, onLogo, onLogoUri, accentColor }) {
  ...
  const handleLogoChange = (v) => {
    onLogo(v)
    if (v === 'custom') fileRef.current?.click()
  }
      ...
      <div className="style-section">
        ...
        <div className="presets-row">
          {QR_PRESETS.map(p => {
            ...
            return (
              <button key={p.id} className={`preset-chip${active ? ' preset-active' : ''}`}
                onClick={() => { onFg(p.fg); onBg(p.bg) }}>
                <div className="preset-swatch">
                  <div style={{ flex: 1, background: p.fg }} />
                  <div style={{ flex: 1, background: p.bg }} />
                </div>
                <span>{p.label}</span>
                {active && <span className="preset-tick">✓</span>}
              </button>
            )
          })}
        </div>
        ...
        {logo === 'brand' && (
          <div className="logo-preview-row">
            ...
            <div className="logo-preview-info">
              <span className="logo-preview-name">Paisa Studio Brand</span>
              <span className="logo-preview-sub">Centered in your QR</span>
            </div>
        ...
        {logo === 'custom' && logoUri && (
          <div className="logo-preview-row logo-preview-row-btn" onClick={() => fileRef.current?.click()}>
            ...
            <div className="logo-preview-info">
              ...
              <span className="logo-preview-sub" style={{ color: 'var(--accent)' }}>Click to change</span>
            </div>
...
export default function App() {
  ...
  const handleQRDetected = useCallback((raw) => {
    ...
    if (esewa) {
      setTab('esewa')
      if (esewa.id) setEsewaId(esewa.id)
      if (esewa.name) setEsewaName(esewa.name)
      return
    }
  ...
  return (
    <div className="app">
      ...
      <nav className="tab-bar">
        ...
        <button className="tab-btn tab-btn-myqrs" disabled>
          <Icon name="grid-outline" size={16} color="var(--text3)" />
          My QRs
        </button>
      </nav>
            ...
            <section className="section-card">
              <div className="section-header">
                ...
                <div className="scan-pill" style={{ background: accentColor + '18', borderColor: accentColor + '40', color: accentColor }}
                  ...
                  <Icon name="scan-outline" size={13} color={accentColor} />
                  <span>Scan</span>
                </div>
              </div>
            ...
            <section className="section-card">
              <div className="section-header">
                ...
                <div className="style-badge" style={{ background: 'var(--accent-light)', borderColor: 'var(--accent-border)', color: 'var(--accent)' }}>
                  <Icon name="color-palette-outline" size={11} color="var(--accent)" />
                  Style
                </div>
              </div>
              <StylePanel fg={fg} bg={bg} dotStyle={dotStyle} cornerStyle={cornerStyle}
                ...
                onLogo={setLogo} onLogoUri={setLogoUri}
